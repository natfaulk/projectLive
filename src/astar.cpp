...
void ASTAR_PrintOpen(void)
{
  for (std::size_t i = 0 ; i < open.size(); i++)
  {
    printf("Open list %lu: x %f, y %f", (unsigned long)i, allPoints.at(open.at(i)).x, allPoints.at(open.at(i)).y);
  }
}
...
void ASTAR_PrintClosed(void)
{
  ...
  {
    printf("Closed list %lu: x %f, y %f", (unsigned long)i, allPoints.at(closed.at(i)).x, allPoints.at(closed.at(i)).y);
  }
}
